...
<head>
    ...
    <meta charset="UTF-8">
    <!-- Kembalikan ke device-width standar agar tidak dipaksa zoom-out berantakan oleh browser HP -->
...
<body class="bg-slate-100 text-slate-800 antialiased min-h-screen flex flex-col">
    ...
    <!-- WRAPPER UTAMA: Mengunci lebar minimal 1200px agar di HP bentuknya sama persis seperti di laptop (bisa digeser horizontal) -->
    <div class="w-full overflow-x-auto flex-1 flex">
        <div class="min-w-[1200px] flex-1 flex flex-row">
            ...
            <aside class="w-72 bg-gradient-to-b from-emerald-950 via-slate-900 to-teal-950 text-white flex flex-col justify-between shadow-2xl shrink-0 border-r border-emerald-800/40 min-h-screen">
                <div>
                    <!-- Brand Header -->
            ...
            <main class="flex-1 p-8 overflow-y-auto">
                ...
                <div class="flex justify-between items-center flex-wrap gap-4 mb-8 bg-white p-6 rounded-3xl shadow-sm border border-slate-200/80">
                    <div>
                        ...
                        <h1 class="text-2xl font-extrabold text-slate-900 tracking-tight">Sistem Perizinan Akses Kawasan Konservasi</h1>
                    </div>
                            ...
                            <div class="mb-10">
                                ...
                                <div class="grid grid-cols-2 gap-6">
                                    ...
                                    <div id="mahasiswa-fields" style="display: none; grid-column: span 2;">
                                        <div class="grid grid-cols-2 gap-6 p-6 bg-emerald-50/50 rounded-2xl border border-emerald-200/60 mb-2">
                                ...
                                <tbody class="divide-y divide-slate-100">
                                    ...
                                    <tr class="hover:bg-slate-50/80 transition-all">
                                        ...
                                        <td class="p-4">
                                            ...
                                            @if($status == 'pending')
                                                <span class="inline-flex items-center gap-1 px-3 py-1.5 rounded-xl text-xs font-bold bg-amber-100 text-amber-800 border border-amber-200">
                                                    <i class="fa-solid fa-spinner fa-spin"></i> Dalam Proses
                                                </span>
                                            @elseif($status == 'disetujui')
                                                <div class="flex flex-col gap-2 items-start">
                                                    <span class="inline-flex items-center gap-1 px-3 py-1.5 rounded-xl text-xs font-bold bg-emerald-100 text-emerald-800 border border-emerald-200">
                                                        <i class="fa-solid fa-circle-check"></i> Disetujui
                                                    </span>
                                                    @if(!empty($item->surat_izin))
                                                        <a href="{{ route('simaksi.downloadPdf', $item->id) }}" class="inline-flex items-center gap-1 bg-teal-600 hover:bg-teal-700 text-white px-3 py-1.5 rounded-xl text-xs font-bold shadow-sm transition-all" target="_blank">
                                                            <i class="fa-solid fa-file-pdf"></i> Unduh Surat
                                                        </a>
                                                    @endif
                                                </div>
                                            @elseif($status == 'ditolak')
                                                <span class="inline-flex items-center gap-1 px-3 py-1.5 rounded-xl text-xs font-bold bg-red-100 text-red-700 border border-red-200">
                                                    <i class="fa-solid fa-circle-xmark"></i> Ditolak
                                                </span>
                                            @endif
                                        </td>
                                        <td class="p-4 text-slate-600 text-xs">
                                            <!-- Menampilkan Catatan Admin langsung dari kolom database catatan_admin -->
                                            @if(!empty($item->catatan_admin))
                                                <div class="mb-2 p-2.5 bg-amber-50/80 border border-amber-200/80 rounded-xl text-amber-900">
                                                    <span class="font-bold block text-[10px] uppercase text-amber-800"><i class="fa-solid fa-comment-dots mr-1"></i> Catatan Admin:</span>
                                                    <p class="text-xs mt-0.5">{{ $item->catatan_admin }}</p>
                                                </div>
                                            @endif
                                            ...
                                            @else
                                                @if(empty($item->catatan_admin))
                                                    <span class="text-slate-400 italic">Belum ada catatan/jadwal</span>
                                                @endif
                                            @endif
                                        </td>
                                    </tr>
                ...
                <div id="tab-content-profile" class="bg-white rounded-3xl shadow-xl shadow-slate-200/60 border border-slate-200/80 overflow-hidden mb-12" style="display: none;">
                    ...
                    <div class="p-10">
                        <div class="flex items-center gap-6 bg-gradient-to-r from-emerald-50 via-teal-50/30 to-white p-8 rounded-3xl mb-8 border border-emerald-200/80 shadow-md">
                            ...
                            <div>
                                ...
                                <h3 class="text-2xl font-extrabold text-emerald-950 mt-1">{{ Auth::user()->name ?? 'Hadynata Yusuf Pratama' }}</h3>
